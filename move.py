...
class Move:
    ...
    def __init__(self, move_data):
        ...
        self.battleStageMod = move_data.get('battleStageMod')
        self.moveStatus = move_data.get('moveStatus')
        self.pp = self.maxpp

    @staticmethod
    def createMove(name: str) -> "Move":
        base_path = os.path.join(os.path.dirname(__file__), "..", "..", "assets", "json", "moves")
        path = os.path.abspath(os.path.join(base_path, f"{name.lower()}.json"))
